<?php
/*
 * WRI template for page thumbnails
...
if (have_posts()) {
	...
	$wri_thumbnail_columns_number = nvl( $wri_thumbnail_columns_number, 4 );
	...
	while (have_posts()) {
		...
		if ( 0 == $i % $wri_thumbnail_columns_number ) {
			$position_in_row = 'last_in_row';
		} else if ( 0 == ($i-1) % $wri_thumbnail_columns_number ) {
			$position_in_row = 'first_in_row';
		} else {
			$position_in_row = '';  
		}

		$output .= "<li class='wri-thumbnail " .  $position_in_row . "'>"; 
		...
		if ( has_post_thumbnail() ) {
			...
			$post_thumbnail_html = get_the_post_thumbnail( null, array(150, 150) ); 
		}
